...
From the package folder: python3 -m unittest discover -s tests -v
"""
...
spec = importlib.util.spec_from_file_location("runner", ROOT / "generate_icons_comfy_v2.py")
...
class MockComfy:
    def __init__(self):
        ...
        buf = io.BytesIO()
        Image.new("RGB", (1024, 1024), (210, 215, 220)).save(buf, "PNG")
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if path == "/object_info":
                    info = {v["class_type"]: {"input": {"required": {}}} for v in r.EMBEDDED_WORKFLOW.values()}
                    for cls, field, value in (("UNETLoader", "unet_name", r.DEFAULT_MODEL), ("CLIPLoader", "clip_name", "qwen_3_4b.safetensors"), ("VAELoader", "vae_name", "flux2-vae.safetensors")):
                        info[cls]["input"]["required"][field] = [[value] if not owner.missing_model else []]
                    self.send_json(info)
                elif path == "/queue":
                    self.send_json({"queue_running": owner.running, "queue_pending": owner.pending})
                elif path.startswith("/history/"):
                    pid = path.split("/")[-1]
                    self.send_json({pid: owner.history[pid]} if pid in owner.history else {})
                elif path == "/view":
            ...
            def do_POST(self):
                ...
                if self.path == "/prompt":
                    ...
                    if owner.behaviour == "success":
                        owner.history[pid] = {"status": {"status_str": "success", "completed": True}, "outputs": {"16": {"images": [{"filename": pid + ".png", "subfolder": "mock", "type": "output"}]}}}
                    elif owner.behaviour == "error":
                        owner.history[pid] = {"status": {"status_str": "error", "completed": False, "messages": [["execution_error", {"exception_message": "mock out of memory"}]]}, "outputs": {}}
                    elif owner.behaviour == "timeout":
                        owner.running.append([0, pid, payload["prompt"], {}, ["16"]])
                    self.send_json({"prompt_id": pid, "node_errors": {}})
                elif self.path == "/interrupt":
                    owner.interrupts.append(payload)
                    owner.running = [row for row in owner.running if row[1] != payload.get("prompt_id")]
                    self.send_response(200)
                    self.end_headers()  # deliberately empty body
                elif self.path == "/queue":
                    owner.deletions.append(payload)
                    owner.pending = [row for row in owner.pending if row[1] not in payload.get("delete", [])]
                    self.send_response(200)
                    self.end_headers()
                else:
                    self.send_json({"error": "not found"}, 404)
...
def example(name="test_icon.png"):
    return {"filename": name, "display_name": "Test icon", "semantic_key": "ui.item.test_icon", "positive_prompt": "One fantasy item on a white background.", "negative_prompt": "", "migration_status": "pending"}


class UnitTests(unittest.TestCase):
    def setUp(self):
        self.graph = copy.deepcopy(r.EMBEDDED_WORKFLOW)
    ...
    def test_literal_seeds_and_suffixes(self):
        for i, seed in enumerate((101, 1012, 103), 1):
            graph = r.build_workflow(self.graph, self.nodes, example(), seed, 1024, "test")
            self.assertEqual(graph["9"]["inputs"]["noise_seed"], seed)
            self.assertEqual(graph["13"]["inputs"]["batch_size"], 1)
            self.assertEqual(r.candidate_name("normal_icon_name.png", i), f"normal_icon_name_{i}.png")
        self.assertEqual(self.graph["9"]["inputs"]["noise_seed"], 101)
    def test_prompt_written_to_primitive(self):
        entry = example()
        graph = r.build_workflow(self.graph, self.nodes, entry, 101, 512, "test")
    ...
    def test_size_updates_scheduler_and_latent(self):
        graph = r.build_workflow(self.graph, self.nodes, example(), 103, 768, "test")
        for node in ("12", "13"):
    ...
    def test_seed_validation(self):
        ...
        self.assertEqual(r.validate_seeds([101, 1012, 103]), [101, 1012, 103])
    ...
    def test_duplicate_and_unsafe_filenames(self):
        ...
        for filename in ("../a.png", "/a.png", "a\\b.png", "x:xx.png", "a.png\n", "a.jpg"):
            with self.assertRaises(r.IconError):
                r.validate_entries([example(filename)])
    def test_base_model_is_rejected(self):
        self.graph["1"]["inputs"]["unet_name"] = "flux-2-klein-base-4b.safetensors"
        with self.assertRaises(r.IconError):
    ...
    def test_fingerprint_ignores_output_prefix_not_seed(self):
        a = r.build_workflow(self.graph, self.nodes, example(), 101, 1024, "first")
        b = r.build_workflow(self.graph, self.nodes, example(), 101, 1024, "second")
        c = r.build_workflow(self.graph, self.nodes, example(), 103, 1024, "second")
        self.assertEqual(r.fingerprint(a, self.nodes, 120), r.fingerprint(b, self.nodes, 120))
        self.assertNotEqual(r.fingerprint(a, self.nodes, 120), r.fingerprint(c, self.nodes, 120))
    def test_manifest_all_entries(self):
        prompts = r.validate_entries(r.load_json(ROOT / "prompts_cartoon_v2.json"))
        self.assertEqual(len(prompts), 121)
        self.assertEqual(len({x["filename"] for x in prompts}), 121)
        self.assertTrue(all(x["negative_prompt"] == "" for x in prompts))
        self.assertTrue(all(x["generation_seeds"] == [101, 1012, 103] for x in prompts))
        for item in prompts:
            for term in ("CURRENT ITEM ONLY:", "repository asset:", "Required game filename", "five finger shapes and one thumb", "sourceScale"):
                self.assertNotIn(term, item["positive_prompt"])
    def test_filters_and_limit(self):
        args = r.parse_args(["--only", "test_icon", "--limit", "1"])
        selected = r.select_entries([example(), example("other.png")], args)
        self.assertEqual(len(selected), 1)
        self.assertEqual(args.seeds, [101, 1012, 103])
    def test_dry_run_does_not_need_server(self):
        with tempfile.TemporaryDirectory() as tmp, contextlib.redirect_stdout(io.StringIO()) as output:
            code = r.main(["--dry-run", "--limit", "1", "--comfy-url", "http://127.0.0.1:1", "--output-root", tmp + "/absent"])
...
class IntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.args = [str(self.prompts), "--output-root", str(self.root / "results"), "--comfy-url", self.server.url, "--poll-seconds", "0.01"]
    def tearDown(self):
        self.server.close()
        self.tmp.cleanup()
    ...
    def test_three_separate_outputs_and_resume(self):
        ...
        for variant, seed in enumerate((101, 1012, 103), 1):
            self.assertEqual(self.server.graphs[variant-1]["9"]["inputs"]["noise_seed"], seed)
            source = self.root / "results/source" / f"test_icon_{variant}.png"
            preview = self.root / "results/preview_120" / source.name
            self.assertTrue(source.exists())
            self.assertEqual(r.inspect_image(preview)["dimensions"], [120, 120])
            self.assertFalse(r.inspect_image(source)["real_transparency"])
        self.assertFalse((self.root / "results/source/test_icon.png").exists())
        self.assertEqual(self.run_script(), 0)
        self.assertEqual(len(self.server.graphs), 3)
        report = r.load_json(self.root / "results/reports/latest_run.json")
    ...
    def test_reconstruct_missing_preview_without_generation(self):
        self.assertEqual(self.run_script(), 0)
        preview = self.root / "results/preview_120/test_icon_2.png"
        preview.unlink()
        self.assertEqual(self.run_script(), 0)
    ...
    def test_history_recovery_without_resubmitting(self):
        self.assertEqual(self.run_script(), 0)
        path = self.root / "results/metadata/test_icon_2.json"
        data = r.load_json(path)
        data["phase"] = "queued"
        r.save_json(path, data)
        (self.root / "results/source/test_icon_2.png").unlink()
        self.assertEqual(self.run_script(), 0)
        self.assertEqual(len(self.server.graphs), 3)
        self.assertTrue((self.root / "results/source/test_icon_2.png").exists())
    def test_timeout_stops_after_one_request_and_targets_id(self):
        ...
        self.assertEqual(self.server.interrupts, [{"prompt_id": "mock-1"}])
        self.assertEqual(self.server.deletions, [])
        self.assertFalse((self.root / "results/.runner.lock").exists())
    ...
    def test_server_check_has_no_generation(self):
        ...
        self.assertEqual(self.server.graphs, [])
    def test_alpha_guard_before_generation(self):
        self.assertEqual(self.run_script(["--require-alpha"]), 1)
        self.assertEqual(self.server.graphs, [])
    def test_pending_cancel_only_own_id(self):
        self.server.pending = [[0, "own", {}, {}, []], [1, "other", {}, {}, []]]
